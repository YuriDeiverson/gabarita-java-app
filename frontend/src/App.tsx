...
const hasActiveStudyPlan = () =>
  Boolean(
    (() => {
      ...
      let examDate = '';
      try {
        examDate = JSON.parse(localStorage.getItem(`${courseId}_study_config`) || '{}').examDate || '';
      } catch {}
      const now = new Date();
      const today = new Date(now.getTime() - now.getTimezoneOffset() * 60_000).toISOString().slice(0, 10);
      return Boolean(courseId && examDate >= today && localStorage.getItem(`${courseId}_study_config`));
    })()
  );
...
const settingsFromPlan = (plan: StudyPlan) => {
  try {
    ...
    return typeof root.preferences === 'object' && root.preferences
...
const isPrimaryPlan = (plan: StudyPlan) => plan.is_primary === true || plan.is_active === true || plan.is_active === 1;
...
export default function App() {
  ...
  const [adminMenuOpen, setAdminMenuOpen] = useState(() => activeTab === 'admin');
  const [adminSection, setAdminSection] = useState<AdminSection>(
    () => (localStorage.getItem('admin_active_section') as AdminSection) || 'contests'
  );

  const hydrateActivePlan = useCallback(
    (plan: StudyPlan) => {
      ...
      if (selectedWeekdays.length > 0) {
        const rawHours =
          typeof settings.hoursByWeekday === 'object' && settings.hoursByWeekday
  ...
  const loadHeaderStudyData = useCallback(async () => {
    ...
    try {
      ...
      if (response.notifications?.length)
        setHeaderNotifications(items => (items.length ? items : response.notifications.filter(item => !item.read_at)));
    } catch {
  ...
  useEffect(() => {
    ...
    else if (!studyPreferences && activeTab !== 'home' && activeTab !== 'notes' && !(isAdmin && activeTab === 'admin'))
      setActiveTab('home');
    else if (
      !hasPlan &&
      activeTab !== 'home' &&
  ...
  const completeCurrentActivity = useCallback(async () => {
    ...
    if (!session.id) {
      ...
      const task = today.tasks.find(
        item =>
          item.roadmap_topic_id === studyContext?.roadmapTopicId && ['AVAILABLE', 'IN_PROGRESS'].includes(item.status)
      );
    ...
    if (session.id) {
      if (session.session_kind === 'QUESTIONS')
        throw new Error('Finalize o Pomodoro do banco de questões antes de concluir este assunto.');
      if (
  ...
  useEffect(() => {
    if (!hasPlan || studyContext || activeTab !== 'study') return;
    dailyStudyApi
      .today()
      .then(response => {
  ...
  const mobileHeaderTitle =
    hasPlan && activeTab === 'home' ? `${timeGreeting}, ${normalizedFirstName}` : activeNavigationItem.label;
  ...
  const headerBreakMinutes =
    headerCycle > 0 && headerCycle % Math.max(1, Number(headerPomodoro?.cycles || 4)) === 0
  ...
  useEffect(() => {
    ...
    const completedTitle = String(activeHeaderSession.topic_title || 'Sessão atual');
    dailyStudyApi
  ...
  const pauseContextQuestionSession = async () => {
    ...
    try {
      const updated = await dailyStudyApi.pause(String(activeHeaderSession.id), 'Pausa manual');
      setHeaderStudyData(current => (current ? { ...current, active_session: updated } : current));
      setHeaderTimerLoadedAt(Date.now());
    } catch (error) {
      setContextSessionError(error instanceof Error ? error.message : 'Não foi possível pausar.');
    } finally {
  ...
  const resumeContextQuestionSession = async () => {
    ...
    try {
      ...
      setContextSessionError(error instanceof Error ? error.message : 'Não foi possível continuar.');
    } finally {
      setContextSessionBusy(false);
    }
  };
  ...
  const finishContextQuestionSession = async () => {
    ...
    setContextSessionError('');
    try {
      await dailyStudyApi.finishQuestionPractice(String(activeHeaderSession.id));
      ...
      setContextSessionError(error instanceof Error ? error.message : 'Não foi possível finalizar.');
    } finally {
      setContextSessionBusy(false);
    }
  };
    ...
    activeHeaderSession && (
      ...
      >
        ...
        <span className="global-timer-state">
          {headerIsBreak ? 'Intervalo' : activeHeaderSession.status === 'PAUSED' ? 'Pausado' : 'Em andamento'}
        </span>
      </div>
    );
  ...
  const openPlanManager = () => {
    ...
    const loadPlans = async () => {
      try {
        ...
        if (plansLoadRequestRef.current !== requestId) return;
        setPlansLoadError('Carregando seu plano de estudos…');
        window.setTimeout(() => void loadPlans(), loadRetryDelay(failedAttempts++));
      }
    };

    void loadPlans();
  };
  const openProfileDestination = (tab: 'career' | 'schedule' | 'performance' | 'notes' | 'admin') => {
    setProfileMenuOpen(false);
    setMobileProfileOpen(false);
    setNotificationMenuOpen(false);
    setActiveTab(tab);
  ...
  const profileContent = () => (
    <>
      ...
      {hasPlan && headerStudyData && (
        <section className="profile-study-overview" aria-label="Progresso de estudos">
          <div className="profile-progress-heading">
            <div>
              <small>Progresso de hoje</small>
              <strong>{Math.round(Number(headerStudyData.today.progress_percentage || 0))}%</strong>
            </div>
          ...
          <div className="profile-study-grid">
            <article className="profile-streak-card">
              ...
              <div>
                ...
                <p>
                  <ShieldCheck />
                  {Number(headerStudyData.streak.protection_balance || 0)} proteções disponíveis
                </p>
              </div>
            </article>
            <article>
              ...
              <div>
                ...
                <p>{Number(headerStudyData.experience.total_xp || 0)} XP acumulados</p>
              </div>
            </article>
            <article>
              <span>
              ...
              <div>
                ...
                <p>{formatMinutes(headerStudyData.today.remaining_minutes)} restantes</p>
              </div>
            </article>
          </div>
        </section>
      ...
      <nav className="profile-destinations" aria-label="Áreas do perfil">
        <button type="button" onClick={() => openProfileDestination('notes')}>
          <span>
            <NotebookPen />
            <span>
        ...
        {hasPlan && (
          <>
            <button type="button" onClick={() => openProfileDestination('performance')}>
              <span>
                <ChartNoAxesCombined />
                <span>
            ...
            <button type="button" onClick={() => openProfileDestination('career')}>
              <span>
                <BriefcaseBusiness />
                <span>
  ...
  const accountActions = (mobile = false) => (
    <div className={mobile ? 'mobile-account-actions' : 'app-account'}>
      {!mobile && hasPlan && activeHeaderSession?.session_kind !== 'QUESTIONS' && globalSessionTimer()}
      {hasPlan && (
        <div
        ...
        {notificationMenuOpen && (
          ...
          >
            ...
            <div className="notification-list">
              ...
              {!notificationLoading && !notificationError && headerNotifications.length === 0 && (
                <p className="empty-note">Tudo em dia. Você não possui notificações pendentes.</p>
              )}
            ...
            {headerNotifications.length > 0 && (
              <div className="notification-panel-footer">
                <button
                  type="button"
                  className="read-all"
                  disabled={unreadNotifications === 0 || markingAllNotifications}
  ...
  const contextualCurrentTask = activeHeaderSession?.daily_task_id
    ? headerStudyData?.tasks?.find(task => task.id === activeHeaderSession.daily_task_id)
    : undefined;
  const contextualScheduledCurrentTask =
    contextualCurrentTask ||
    headerStudyData?.tasks?.find(task => ['AVAILABLE', 'IN_PROGRESS'].includes(task.status)) ||
  ...
  const contextualCurrentTaskIndex = contextualScheduledCurrentTask
    ? (headerStudyData?.tasks?.findIndex(task => task.id === contextualScheduledCurrentTask.id) ?? -1)
    : -1;
  const contextualHomeNextTask = headerStudyData?.tasks?.find(
    (task, index) => index > contextualCurrentTaskIndex && !['COMPLETED', 'SKIPPED'].includes(task.status)
  );
  ...
  const contextualProgress = Math.max(
    0,
    Math.min(100, Math.round(Number(headerStudyData?.today?.progress_percentage || 0)))
  );
  const contextRail = showContextRail && (
    <aside className="app-context-rail" aria-label="Resumo e atalhos contextuais">
      ...
      {hasPlan ? (
        <>
          {activeTab === 'home' && (
            <>
              <section className="context-next-card context-home-card" aria-label="Próxima atividade">
                ...
                <p>
                  {String(
                    contextualHomeNextTask?.subject_name || 'Conclua a sessão atual para atualizar a próxima atividade.'
                  )}
                </p>
              </section>
                ...
                <p>
                  {String(
                    contextualAlert?.message ||
                      (contextualExamDate
                        ? `Prova em ${contextualExamDate.split('-').reverse().join('/')}.`
                        : 'Defina a data da prova para receber alertas de prazo.')
                  )}
                </p>
              </section>
            </>
          )}
          <section className="context-primary-card context-default-card" aria-label="Preparação ativa">
            ...
            <div className="context-exam-row">
              ...
              <div>
                ...
                <strong>{contextualExamDate ? contextualExamDate.split('-').reverse().join('/') : 'A definir'}</strong>
              </div>
              {contextualExamDays !== null && (
                <b>
                  {contextualExamDays < 0
          ...
          <section className="context-progress-card context-default-card" aria-label="Progresso de hoje">
            <div className="context-card-heading">
              <div>
          ...
          <section className="context-next-card context-default-card" aria-label="Próxima atividade">
            <div className="context-card-heading">
              <div>
                <span>Próxima atividade</span>
                <strong>{contextualNextTask ? 'Pronta para começar' : 'Rotina concluída'}</strong>
              </div>
              <BookOpen aria-hidden="true" />
            </div>
            <h3>
              {String(contextualNextTask?.topic_title || headerStudyData?.next?.title || 'Nenhuma atividade pendente')}
            </h3>
            ...
            {contextualNextTask && (
              ...
              >
                ...
                {contextualNextTask.activity_type === 'QUESTIONS' ? 'Fazer questões' : 'Continuar estudando'}
              </button>
            )}
          </section>

          {activeHeaderSession && (
            <section className="context-session-card context-default-card" aria-label="Sessão em andamento">
              <span>
                <Clock3 aria-hidden="true" />
              </span>
              <div>
                <small>{headerIsBreak ? 'Intervalo atual' : 'Sessão em andamento'}</small>
                <strong>{formatClock(headerTimerSeconds)}</strong>
                <p>
                  {String(activeHeaderSession.topic_title || activeHeaderSession.context_title || 'Atividade atual')}
                </p>
              </div>
              {activeHeaderSession.session_kind === 'QUESTIONS' && (
  ...
  return (
    ...
    >
          ...
          <div className="desktop-header-content flex items-center justify-between min-h-16 py-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-0.5">
                {isHome ? (
                  <>
                    <h1 className="text-base font-extrabold tracking-tight text-slate-900">Gabaritando Concursos</h1>
                    <p className="text-[10px] text-slate-500 font-medium">Planejamento enxuto para reta final</p>
                  </>
                ) : (
                  <>
                    ...
                    <p className="text-[10px] text-slate-500 font-medium">{brand.subtitle}</p>
                  </>
                )}
              </div>
            </div>
      ...
      <main id="main-content" className="app-main" tabIndex={-1}>
        ...
        {(hasPlan || studyPreferences || isAdmin) && (
          ...
          >
            ...
            {navigationItems.map(item => {
              ...
              if (item.id === 'admin')
                return (
                  <div key={item.id} className={`sidebar-admin-group ${adminMenuOpen ? 'is-open' : ''}`}>
                    <button
                      type="button"
                      id="tab-admin-trigger"
                      onClick={() => {
                        setAdminMenuOpen(open => (sidebarCollapsed ? true : !open));
                    ...
                    {adminMenuOpen && (
                      <div className="sidebar-admin-submenu" aria-label="Seções administrativas">
                        {adminNavigationItems.map(adminItem => {
                          ...
                          return (
                            <button
                              ...
                              aria-current={activeTab === 'admin' && adminSection === adminItem.id ? 'page' : undefined}
                            >
                              <AdminIcon />
        ...
        <div className="app-content min-w-0 flex-grow transition-all duration-300">
          <Suspense
            fallback={
              <section
                className="mx-auto flex min-h-64 max-w-4xl items-center justify-center gap-3 text-slate-500"
                role="status"
              >
          ...
          >
              ...
              <PlanManager
                ...
                onDeleted={(_courseId, plan) => {
                  ...
                  if (isPrimaryPlan(plan)) {
                    ...
                    localStorage.setItem('study_plan_deleted', 'true');
                    ['active_course', 'study_config', 'active_study_context'].forEach(key =>
            ...
            {activeTab === 'home' &&
              ...
              (!studyPreferences || editingPreferences) && (
                <InitialStudySetup initial={studyPreferences} onSave={handlePreferencesSave} />
              )}
            {activeTab === 'home' &&
              !plansBootstrapping &&
              studyPreferences &&
              ...
              homeMode === 'dashboard' && (
                <StudyDashboard
                  ...
                  onOpenQuestions={(id, minutes) => {
                    setQuestionDailyTask(id ? { id, minutes: minutes || 30 } : null);
                    setNotedQuestionId('');
                    setActiveTab('questions');
                  }}
            ...
            {activeTab === 'home' &&
              ...
              !hasPlan && (
                <section className="mx-auto max-w-4xl animate-fade-in rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm sm:p-12">
                  ...
                  <h2 className="mt-5 text-2xl font-black text-slate-950">Sua disponibilidade está pronta</h2>
                  <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-slate-500">
                    Agora acesse Concursos, escolha o edital e depois o cargo. Todo o conteúdo será incluído
            ...
            {hasPlan && activeTab === 'study' && (
              <StudyTab studyContext={studyContext} onCurrentActivityComplete={completeCurrentActivity} />
            )}
            ...
            {hasPlan && (
              <QuestionBankTab
                ...
                externalSession={headerStudyData ? activeHeaderSession : undefined}
            ...
            {activeTab === 'notes' && <QuestionNotesTab onOpenQuestion={openNotedQuestion} />}
            {isAdmin && activeTab === 'admin' && (
              <AdminPanel activeSection={adminSection} onSectionChange={setAdminSection} />
            )}
          </Suspense>
        </div>
        {contextRail}
        ...
        <div className="finish-modal-backdrop" role="presentation">
          ...
          >
            ...
            <p id="new-plan-session-description">
              ...
              {newPlanSessionPrompt.topic_title ? ` em “${newPlanSessionPrompt.topic_title}”` : ''}. Para configurar um
              novo plano, essa sessão será cancelada e o cronômetro voltará a zero. Depois, será necessário iniciar
            ...
            <div className="mt-5 flex flex-col-reverse sm:flex-row gap-2 sm:justify-end">
              ...
              >
                {newPlanSessionBusy ? 'Encerrando sessão…' : 'Sair e zerar sessão'}
              </button>
            </div>
          </section>
        </div>
      ...
      {breakNotice && (
        <aside className="pomodoro-break-notice" role="alert" aria-live="assertive">
          ...
          <div>
            ...
            <p>
              O foco em “{breakNotice.title}” terminou. Descanse por {breakNotice.minutes} minutos.
            </p>
          </div>
          <button type="button" onClick={() => setBreakNotice(null)} aria-label="Fechar aviso de descanso">
            <X />
          </button>
        </aside>
      )}
        ...
        <div className="pomodoro-rest-complete-backdrop" role="presentation">
          ...
          >
            ...
            <p id="pomodoro-rest-complete-description">
              Os 10 minutos de descanso terminaram. Deseja iniciar a próxima sessão de foco em “
              {completedBreakPrompt.title}” agora?
            </p>
            {completedBreakError && (
            ...
            <div className="pomodoro-rest-complete-actions">
              ...
              >
                <Play /> {completedBreakBusy ? 'Iniciando…' : 'Ir para a próxima sessão'}
              </button>
            </div>
          </section>
        </div>
      ...
      {mobileProfileOpen && (
        <div
          ...
          role="presentation"
          onMouseDown={event => event.target === event.currentTarget && setMobileProfileOpen(false)}
        >
          <aside
          ...
          >
            ...
            <header>
              ...
              <button type="button" onClick={() => setMobileProfileOpen(false)} aria-label="Fechar menu">
                <X />
              </button>
            </header>
            {profileContent()}
      ...
      <nav className={`mobile-bottom-nav ${hasPlan ? '' : 'is-no-plan'}`} aria-label="Navegação principal mobile">
        {(hasPlan
          ? navigationItems.filter(item => !['career', 'performance', 'admin'].includes(item.id))
